<script lang="ts">
  ...
  const questionTypes = [
    { 
      ...
      questions: [
        { q: "Är du närmare eller längre ifrån Burger King Västerås än mig?", a: "Längre ifrån" },
    ...
    { 
      ...
      questions: [
        { q: "Vilket foto är från detta område?", a: "Det första fotot (överst till vänster)" },
        { q: "Identifiera byggnaden på bilden", a: "Gamla postkontoret från 1920" },
        { q: "Var togs detta foto?", a: "Från Västra Torggatan" },
        { q: "Vilken fasad tillhör denna byggnad?", a: "Huvudfasaden mot väster" },
    ...
    { 
      ...
      questions: [
        ...
        { q: "Vilken är annorlunda?", a: "Cykeln (övriga är fordon med motor)" },
        { q: "Vilket tillhör inte gruppen?", a: "Trädet (övriga är byggnader)" },
        { q: "Vilket är det felaktigaste?", a: "Det gula skylten (övriga är vita)" },
        { q: "Vad bryter mönstret?", a: "Den moderna byggnaden (övriga är gamla)" },
        { q: "Vilket är från en annan tid?", a: "Bilen från 1960-talet" },
        { q: "Vad är överflödigt?", a: "Affischen (den är inte originalbyggnaden)" },
        { q: "Vilket material sticker ut?", a: "Glasfasaden (övriga är sten)" },
        { q: "Vilken färg passar inte?", a: "Grön (övriga är i varma färger)" },
        { q: "Vilket är från annan byggnad?", a: "Dörren från andra sidan vägen" },
    ...
    { 
      ...
      questions: [
        ...
        { q: "Markera den exakta positionen", a: "Vid huvudingången på västra sidan" },
  ...
  function submitAnswer() {
    if (!isAnswered) {
      ...
      dispatch('questionAnswered', { type: currentType, questionIndex: selectedQuestion });
    }
  }
...
<div class="relative overflow-hidden rounded-3xl">
  <div
    ...
    style="opacity: {slideDirection === 0 ? 1 : 0}; transform: translateX({slideDirection * 50}px);"
  >
    <div class="flex justify-between items-center mb-4">
      <div class="flex items-center gap-3">
        <button on:click={prevType} disabled={isTransitioning} class="text-2xl font-bold {current.accentColor} hover:opacity-70 transition-opacity disabled:opacity-50">‹</button>
        <span class="text-sm font-semibold {current.accentColor} min-w-32">{current.name}</span>
        <button on:click={nextType} disabled={isTransitioning} class="text-2xl font-bold {current.accentColor} hover:opacity-70 transition-opacity disabled:opacity-50">›</button>
      </div>
      <span class="text-sm font-semibold {current.accentColor}">{scoreChange}</span>
    </div>
    ...
    <div class="grid grid-cols-6 gap-2 mb-4">
      {#each Array(12) as _, i}
        <button
          ...
          class="aspect-square {answeredQuestions[currentType].has(i) ? current.lightButtonColor : current.buttonColor} rounded-xl hover:opacity-80 flex items-center justify-center text-lg transition-all duration-200 cursor-pointer border-2 {selectedQuestion === i ? current.accentColor + ' border-current' : 'border-transparent'}"
        >
          {current.icon}
        </button>
      {/each}
    ...
    <div class="mb-4">
      <p class="text-xs font-medium {current.accentColor} mb-2">Q: {currentQuestion.q}</p>
      {#if isAnswered}
        <div class="bg-white rounded-xl p-3 border-l-4 {current.accentColor.replace('text-', 'border-')}">
          <p class="text-xs font-semibold {current.accentColor} mb-1">✓ Korrekt svar:</p>
          <p class="text-sm font-bold {current.accentColor}">{currentQuestion.a}</p>
        </div>
      {/if}
    </div>
